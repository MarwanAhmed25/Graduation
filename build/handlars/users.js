    ...
    try {
        ...
        if (user_type == 'user') {
            ...
            if (req.body.role) {
                ...
                else if (req.body.role == 'admin' && user_type === 'super_admin') {
                    user_.role = req.body.role;
                }
            }
        }
...
async function get_token(req, res) {
    ...
    try {
        //check if the request from super admin?
        const isAdmin = (0, isAdmin_1.default)(admin_email, admin_password, token);
        if (isAdmin) { //if request from admin user or super admin will return token for user with id of request id
            ...
            const res_token = jsonwebtoken_1.default.sign({ user: res_user }, secret);
            res.status(200).json(res_token);
        }
        else
            throw new Error('not allowed.'); //else return not allowed        
    }
    catch (e) {
        res.status(400).json(`${e}`);
    }
